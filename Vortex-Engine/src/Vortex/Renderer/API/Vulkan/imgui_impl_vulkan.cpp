                                     ...
                                     VkPipeline      pipeline)
{
    ...
    if (wrb->FrameRenderBuffers == nullptr)
    {
        ...
        wrb->FrameRenderBuffers
            = (ImGui_ImplVulkan_FrameRenderBuffers*)IM_ALLOC(
...
bool ImGui_ImplVulkan_CreateFontsTexture()
{
    ...
    if (bd->FontCommandPool == VK_NULL_HANDLE)
    {
        ...
        vkCreateCommandPool(v->Device, &info, v->Allocator,
                            &bd->FontCommandPool);
    }
    if (bd->FontCommandBuffer == VK_NULL_HANDLE)
    {
        ...
        err                     = vkAllocateCommandBuffers(v->Device, &info,
    ...
    VkBuffer       upload_buffer;
    {
        ...
        check_vk_result(err);
        err = vkBindBufferMemory(v->Device, upload_buffer, upload_buffer_memory,
                                 0);
        check_vk_result(err);
    }
    ...
    // Copy to Image:
    {
        ...
        vkCmdCopyBufferToImage(
            bd->FontCommandBuffer, upload_buffer, bd->FontImage,
                                     ...
                                     bool mainWindow = false)
{
    ...
    if (bd->ShaderModuleVert == VK_NULL_HANDLE)
    {
        ...
        if (mainWindow)
        {
            vert_info.codeSize = vertShaderCode.size();
            vert_info.pCode    = (const uint32_t*)vertShaderCode.data();
        }
        else
        {
    ...
    if (bd->ShaderModuleFrag == VK_NULL_HANDLE)
    {
        ...
        if (mainWindow)
        {
            frag_info.codeSize = fragShaderCode.size();
            frag_info.pCode    = (const uint32_t*)fragShaderCode.data();
        }
        else
        {
...
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_CreateShaderModules(device, allocator, mainWindow);
    ...
    if (bd->VulkanInitInfo.UseDynamicRendering)
    {
        ...
        IM_ASSERT(bd->VulkanInitInfo.PipelineRenderingCreateInfo.pNext
                      == nullptr
...
bool ImGui_ImplVulkan_CreateDeviceObjects()
{
    ...
    ImGui_ImplVulkan_CreatePipeline(v->Device, v->Allocator, v->PipelineCache,
                                    v->RenderPass, v->MSAASamples,
                                    &bd->Pipeline, v->Subpass, v->mainWindow);
...
static void ImGui_ImplVulkan_CreateWindow(ImGuiViewport* viewport)
{
    ...
    #ifdef IMGUI_IMPL_VULKAN_HAS_DYNAMIC_RENDERING
    for (uint32_t n = 0;
         n < v->PipelineRenderingCreateInfo.colorAttachmentCount; n++)
    ...
    {
        {
            ...
            {
                // Since we are not going to swap this frame anyway, it's ok
                // that recreation happens on next frame.
