...
namespace Vortex
{
    ...
    inline void SetupDarkImGuiStyle()
    {
        ...
        PM_UNUSED constexpr auto accentColor     = IM_COL32(236, 158, 36, 255);
        constexpr auto           highlightColor  = IM_COL32(39, 185, 242, 255);
        PM_UNUSED constexpr auto niceBlueColor   = IM_COL32(83, 232, 254, 255);
        PM_UNUSED constexpr auto complimentColor = IM_COL32(78, 151, 166, 255);
        ...
        PM_UNUSED constexpr auto titlebarOrangeColor
            = IM_COL32(186, 66, 30, 255);
        PM_UNUSED constexpr auto titlebarGreenColor = IM_COL32(18, 88, 30, 255);
        PM_UNUSED constexpr auto titlebarRedColor = IM_COL32(185, 30, 30, 255);
        constexpr auto           propertyFieldColor = IM_COL32(15, 15, 15, 255);
        constexpr auto           textColor = IM_COL32(192, 192, 192, 255);
        PM_UNUSED constexpr auto textBrighterColor
            = IM_COL32(210, 210, 210, 255);
        PM_UNUSED constexpr auto textDarkerColor = IM_COL32(128, 128, 128, 255);
        PM_UNUSED constexpr auto textErrorColor  = IM_COL32(230, 51, 51, 255);
        PM_UNUSED constexpr auto mutedColor      = IM_COL32(77, 77, 77, 255);
        constexpr auto           groupHeaderColor = IM_COL32(47, 47, 47, 255);
        PM_UNUSED constexpr auto selectionColor = IM_COL32(237, 192, 119, 255);
        PM_UNUSED constexpr auto selectionMutedColor
            = IM_COL32(237, 201, 142, 23);
        constexpr auto backgroundPopupColor       = IM_COL32(50, 50, 50, 255);
        PM_UNUSED constexpr auto validPrefabColor = IM_COL32(82, 179, 222, 255);
        PM_UNUSED constexpr auto invalidPrefabColor
            = IM_COL32(222, 43, 43, 255);
        PM_UNUSED constexpr auto missingMeshColor = IM_COL32(230, 102, 76, 255);
        PM_UNUSED constexpr auto meshNotSetColor  = IM_COL32(250, 101, 23, 255);

        // Headers
        colors[ImGuiCol_Header]
            = ImGui::ColorConvertU32ToFloat4(groupHeaderColor);
        ...
        colors[ImGuiCol_FrameBg]
            = ImGui::ColorConvertU32ToFloat4(propertyFieldColor);
        ...
        // Tabs
        colors[ImGuiCol_Tab] = ImGui::ColorConvertU32ToFloat4(titlebarColor);
        colors[ImGuiCol_TabHovered] = ImColor(255, 225, 135, 30);
        colors[ImGuiCol_TabActive]  = ImColor(255, 225, 135, 60);
        colors[ImGuiCol_TabUnfocused]
            = ImGui::ColorConvertU32ToFloat4(titlebarColor);
        ...
        // Text
        colors[ImGuiCol_Text]      = ImGui::ColorConvertU32ToFloat4(textColor);

        // Checkbox
        colors[ImGuiCol_CheckMark] = ImGui::ColorConvertU32ToFloat4(textColor);

        // Separator
        colors[ImGuiCol_Separator]
            = ImGui::ColorConvertU32ToFloat4(backgroundDarkColor);
        colors[ImGuiCol_SeparatorActive]
            = ImGui::ColorConvertU32ToFloat4(highlightColor);
        ...
        colors[ImGuiCol_ChildBg]
            = ImGui::ColorConvertU32ToFloat4(backgroundColor);
        colors[ImGuiCol_PopupBg]
            = ImGui::ColorConvertU32ToFloat4(backgroundPopupColor);
    ...
    void VulkanImGuiLayer::OnAttach()
    {
        ...
        auto      swapChain     = window.SwapChain().As<VulkanSwapChain>();
        vk::Queue graphicsQueue = VulkanRenderer::Device().GraphicsQueue();

        auto      poolSizes     = ToArray<vk::DescriptorPoolSize>(
        ...
        poolInfo.pPoolSizes    = poolSizes.Raw();
        ...
        if (Window::WindowSubsystem() != WindowSubsystem::eWayland)
            io.ConfigFlags
        ...
        auto indices = VulkanRenderer::PhysicalDevice().QueueFamilyIndices();
        ...
        initInfo.Instance = vk::Instance(VulkanRenderer::VulkanInstance());
        initInfo.PhysicalDevice
            = vk::PhysicalDevice(VulkanRenderer::PhysicalDevice());
        ...
        initInfo.Subpass        = 0;
        u32 frameCount          = static_cast<u32>(swapChain->ImageCount());
        ...
        commandPoolInfo.queueFamilyIndex = VulkanRenderer::PhysicalDevice()
                                               .QueueFamilyIndices()
                                               .Graphics.Value();
    ...
    void VulkanImGuiLayer::Begin()
    {
        ...
        auto     windowSize      = bd->MainWindow->Size();

        auto     framebufferSize = bd->MainWindow->FramebufferSize();
        ...
        f64 currentTime = Time::CurrentTime();
    ...
    void VulkanImGuiLayer::End()
    {
        auto&             window    = Application::Get()->Window();
        auto              swapChain = window.SwapChain().As<VulkanSwapChain>();
        vk::CommandBuffer commandBuffer = swapChain->CurrentCommandBuffer();
        ImGui::Render();

#if 0
        auto         currentImageIndex = swapChain.CurrentImageIndex();
        ...
        colorAttachmentInfo.imageView
            = swapChain.Frames()[currentImageIndex].ImageView;
    ...
    void VulkanImGuiLayer::OnUpdate(Timestep) {}
    ...
    void VulkanImGuiLayer::InitializePlatformInterface()
    {
        ...
        io.Platform_GetWindowPos       = WindowPosition;
        io.Platform_SetWindowSize      = SetWindowSize;
        io.Platform_GetWindowSize      = WindowSize;
            ...
            if (data->Window->IsFocused())
            {
                if (io.WantSetMousePos)
                    data->Window->SetCursorPosition(
                        Math::Vec2d((f64)(mousePrevPos.x - viewport->Pos.x),
                                    (f64)(mousePrevPos.y - viewport->Pos.y)));

                if (!bd->MouseWindow)
                {
                    auto cursorPos = data->Window->CursorPosition();
                    if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
                    {
                        ...
                        auto windowPos = data->Window->Position();
                        cursorPos += windowPos;
                    }
                                        ...
                                        u32 repeatCount)
    {
        ...
        usize keycode = ToUnderlying(key);

        if (keycode < bd->KeyOwnerWindows.Size())
    ...
    {
        ImGuiIO& io = ImGui::GetIO();
        if (ToUnderlying(button) < 0
            || ToUnderlying(button) >= ImGuiMouseButton_COUNT)
            return false;
                                        ...
                                        f64 yoffset)
    {
        ...
        if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
        {
            Math::Vec2i pos = window->Position();
            xoffset += pos.x;
            yoffset += pos.y;
        }
    ...
    void VulkanImGuiLayer::DestroyWindow(ImGuiViewport* viewport)
    {
        ...
        {
            if (data->WindowOwned
                && data->Window != &Application::Get()->Window())
            {
    ...
    ImVec2 VulkanImGuiLayer::WindowPosition(ImGuiViewport* viewport)
    {
        auto position = GetViewportData(viewport)->Window->Position();
    ...
    ImVec2 VulkanImGuiLayer::WindowSize(ImGuiViewport* viewport)
    {
        auto size = GetViewportData(viewport)->Window->Size();
    ...
    void VulkanImGuiLayer::SetWindowSize(ImGuiViewport* viewport, ImVec2 size)
    {
        ...
        data->Window->SetSize(Math::Vec2(size.x, size.y));
    }
                                              ...
                                              const void*, ImU64* outVkSurface)
    {
        ...
        VulkanSurface surf;
        surf.Create(data->Window, VulkanRenderer::PhysicalDevice());
