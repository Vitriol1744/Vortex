...
namespace Vortex
{
        ...
        :Texture2D(path)
    {
        ...
        u32            mipLevels  = static_cast<u32>(Prism::Math::Floor(
                            std::log2(Max(image.Width(), image.Height()))))
                      + 1;
        ...
        Memory::Copy(dest, image.Pixels(), size);
        VulkanAllocator::UnmapMemory(stagingAllocation);

        m_TextureImage.Create(image.Width(), image.Height(),
        ...
        m_TextureImage.TransitionLayout(vk::ImageLayout::eUndefined,
                                        ...
                                        mipLevels);
        m_TextureImage.CopyFrom(stagingBuffer, image.Width(), image.Height());
        // TODO(v1tr10l7): Check support for mip mapping
        m_TextureImage.TransitionLayout(vk::ImageLayout::eUndefined,
                                        vk::ImageLayout::eTransferDstOptimal,
                                        mipLevels);
        m_TextureImage.GenerateMipMaps(image.Width(), image.Height(),
                                       mipLevels);
