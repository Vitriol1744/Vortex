...
namespace Vortex
{
    static Array<const char*, 2> s_DeviceExtensions = {
                                         ...
                                         u32                queueFamily)
    {
        ...
        if (Window::WindowSubsystem() == WindowSubsystem::eX11)
        {
            ...
            presentationSupport = vkGetPhysicalDeviceXcbPresentationSupportKHR(
                physicalDevice, queueFamily, X11::Connection(), visualID);
        }
        else if (Window::WindowSubsystem() == WindowSubsystem::eWayland)
        {
            ...
            presentationSupport
                = vkGetPhysicalDeviceWaylandPresentationSupportKHR(
                    physicalDevice, queueFamily, Wayland::Display());
        }
#elifdef VT_PLATFORM_WINDOWS
        vk::Instance instance = VulkanRenderer::VulkanInstance();
    ...
    VulkanPhysicalDevice VulkanPhysicalDevice::Pick()
    {
        ...
        vk::Instance         vkInstance
            = vk::Instance(VulkanRenderer::VulkanInstance());

        u32 gpuCount = 0;
        VkCall(vkInstance.enumeratePhysicalDevices(&gpuCount, VK_NULL_HANDLE));
        VtCoreAssert(gpuCount > 0);
        Vector<vk::PhysicalDevice> physicalDevices(gpuCount);
        ...
        for (auto device : physicalDevices)
        {
            ...
            if (ret.IsDeviceSuitable(device) && score > topScore
                && ret.m_QueueFamilyIndices.Graphics.HasValue()
                && ret.m_QueueFamilyIndices.Present.HasValue())
            {
                bestDevice               = device;
                topScore                 = score;
        ...
        PM_UNUSED const char* deviceName = deviceProperties.deviceName.data();
        PM_UNUSED f64         deviceMemorySize
        ...
        vk::PhysicalDevice physDevice)
    {
        ...
        VkCall(physDevice.enumerateDeviceExtensionProperties(
            nullptr, &extensionCount, availableExtensions.Raw()));

        std::set<String> requiredExtensions(s_DeviceExtensions.begin(),
                                            s_DeviceExtensions.end());

        for (const auto& extension : availableExtensions)
            requiredExtensions.erase(StringView(extension.extensionName));
    ...
    void VulkanDevice::Initialize(const VulkanPhysicalDevice& physicalDevice)
    {
        ...
        struct VulkanPhysicalDevice::QueueFamilyIndices queueFamilyIndices
        ...
        if (queueFamilyIndices.Compute.HasValue())
        ...
        PM_UNUSED vk::PhysicalDeviceDynamicRenderingFeaturesKHR
        ...
        deviceCreateInfo.pNext = VK_NULL_HANDLE; //&dynamicRenderingFeature;
        deviceCreateInfo.flags = vk::DeviceCreateFlagBits();
        deviceCreateInfo.queueCreateInfoCount
            = static_cast<u32>(queueCreateInfos.Size());
        deviceCreateInfo.pQueueCreateInfos = queueCreateInfos.Raw();
        ...
        deviceCreateInfo.ppEnabledExtensionNames = s_DeviceExtensions.Raw();
        ...
        u32 graphicsFamilyIndex
            = m_PhysicalDevice.QueueFamilyIndices().Graphics.Value();
        u32 transferFamilyIndex
            = m_PhysicalDevice.QueueFamilyIndices().Transfer.Value();
