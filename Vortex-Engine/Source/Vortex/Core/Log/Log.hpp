...
namespace Vortex::Log
{
    ...
    {
        GetClientLogger().Trace(format, Forward<Args>(args)...);
    }
    template <typename... Args>
    void Info(fmt::format_string<Args...> format, Args&&... args)
    {
        GetClientLogger().Info(format, Forward<Args>(args)...);
    }
    template <typename... Args>
    void Warn(fmt::format_string<Args...> format, Args&&... args)
    {
        GetClientLogger().Warn(format, Forward<Args>(args)...);
    }
    template <typename... Args>
    void Error(fmt::format_string<Args...> format, Args&&... args)
    {
        GetClientLogger().Error(format, Forward<Args>(args)...);
    }
    template <typename... Args>
    void Fatal(fmt::format_string<Args...> format, Args&&... args)
    {
        GetClientLogger().Fatal(format, Forward<Args>(args)...);
    }
}; // namespace Vortex::Log
