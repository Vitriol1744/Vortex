...
namespace Vortex
{
    ...
    namespace
    {
        ...
        {
            if (display == X11::Display()) errorCode = event->error_code;
        ...
        : Window(specification)
    {
        ...
        PM_UNUSED
        const char* title  = specification.Title.Raw();
        ...
        Visual*      visual
            = DefaultVisual(X11::Display(), DefaultScreen(X11::Display()));
        int depth = DefaultDepth(X11::Display(), DefaultScreen(X11::Display()));
        XSetWindowAttributes attributes{};
        attributes.colormap = XCreateColormap(
            X11::Display(), X11::GetRootWindow(), visual, AllocNone);
        attributes.event_mask        = eventMask;
        attributes.override_redirect = false;

        m_WindowHandle               = XCreateWindow(
            X11::Display(), X11::GetRootWindow(), x, y, width, height, 0, depth,
            InputOutput, visual, CWEventMask | CWBorderPixel | CWColormap,
            &attributes);
        m_InputContext
            = XCreateIC(X11::InputMethod(), XNClientWindow, m_WindowHandle,
        ...
        if (s_WindowsCount == 1)
        {
            ...
            Pixmap blankCursorPixMap = XCreateBitmapFromData(
                X11::Display(), DefaultRootWindow(X11::Display()), data, 1, 1);
            XColor blankCursorColor;
            s_BlankCursor = XCreatePixmapCursor(
                X11::Display(), blankCursorPixMap, blankCursorPixMap,
                &blankCursorColor, &blankCursorColor, 0, 0);

            XFreePixmap(X11::Display(), blankCursorPixMap);
        }
    }
    X11Window::~X11Window()
    {
        m_SwapChain.Reset();
        WindowMap().Erase(m_WindowHandle);
        XDestroyWindow(X11::Display(), m_WindowHandle);
        --s_WindowsCount;
    }
    ...
    bool X11Window::IsHovered() const noexcept
    {
        ...
        while (w)
        {
            ...
            const Bool result
                = XQueryPointer(X11::Display(), w, &root, &w, &rootX, &rootY,
                                &childX, &childY, &mask);
    ...
    String      X11Window::Title() const noexcept { return m_Data.Title; }
    Math::Vec2i X11Window::Position() const noexcept
    {
        Math::Vec2i position;
        ::Window    dummy;
        XTranslateCoordinates(X11::Display(), m_WindowHandle,
                              X11::GetRootWindow(), 0, 0, &position.x,
                              &position.y, &dummy);

        return position;
    }
    inline Math::Vec2i X11Window::Size() const noexcept
    {
        XWindowAttributes attributes{};
        XGetWindowAttributes(X11::Display(), m_WindowHandle, &attributes);

        return Math::Vec2i(attributes.width, attributes.height);
    }
    Math::Vec2i X11Window::FramebufferSize() const noexcept { return Size(); }
    Math::Vec2f X11Window::ContentScale() const noexcept
    {
        VtTodo();
        return {1.0f, 1.0f};
    }
    f32 X11Window::Opacity() const noexcept
    {
        f32 opacity = 1.0f;
    ...
    Math::Vec2d X11Window::CursorPosition() const noexcept
    {
        ::Window root, child;
        i32      rootX, rootY, childX, childY;
        u32      mask;

        XQueryPointer(X11::Display(), m_WindowHandle, &root, &child, &rootX,
                      &rootY, &childX, &childY, &mask);
        return Math::Vec2d(static_cast<f64>(childX), static_cast<f64>(childY));
    }

    void X11Window::Close() noexcept { m_Data.IsOpen = false; }
    void X11Window::RequestFocus() noexcept
    {
        Atom netActiveWindow = X11::Atom("_NET_ACTIVE_WINDOW", true);
        ...
        if (netActiveWindow)
        {
            ...
            VtCoreAssert(XSendEvent(X11::Display(),
                                    DefaultRootWindow(X11::Display()), false,
                                    mask, &event));
        }
        else {
            XSetInputFocus(X11::Display(), m_WindowHandle, RevertToPointerRoot,
                           CurrentTime);
            XRaiseWindow(X11::Display(), m_WindowHandle);
        }
    }
    ...
    void X11Window::SetTitle(StringView title)
    {
        VtUnused(title);
        VtTodo();
    }
    ...
    {
        XMoveWindow(X11::Display(), m_WindowHandle, x, y);
        m_Data.Position.x = x;
        m_Data.Position.y = y;
    }
    ...
    void X11Window::SetSize(const Math::Vec2i& size) noexcept
    {
        VtUnused(size);
        VtTodo();
    }
    void X11Window::SetOpacity(f32 opacity)
    {
        ...
        XChangeProperty(X11::Display(), m_WindowHandle,
                        X11::Atom("_NET_WM_WINDOW_OPACITY"), XA_CARDINAL, 32,
    ...
    void X11Window::SetCursorPosition(Math::Vec2d position) noexcept
    {
        m_WarpCursorPos.x = static_cast<i32>(position.x);
        m_WarpCursorPos.y = static_cast<i32>(position.y);

        XWarpPointer(X11::Display(), None, m_WindowHandle, 0, 0, 0, 0,
                     m_WarpCursorPos.x, m_WarpCursorPos.y);
        XFlush(X11::Display());
    }
    void X11Window::ShowCursor() const noexcept
    {
        XDefineCursor(X11::Display(), m_WindowHandle, None);
    }
    void X11Window::HideCursor() const noexcept
    {
        XDefineCursor(X11::Display(), m_WindowHandle, s_BlankCursor);
    }
    ...
    {
        if (visible) XMapWindow(X11::Display(), m_WindowHandle);
        VtTodo();
    }
    ...
    void X11Window::ProcessEvent(XEvent& event)
    {
        switch (event.type)
        {
            case ButtonPress:
            {
                auto window = WindowMap()[event.xbutton.window];
                if (!window) return;
            ...
            case ClientMessage:
            {
                auto window = WindowMap()[event.xbutton.window];
                if (!window) return;
                static Atom wmProtocols = X11::Atom("WM_PROTOCOLS");
                if (event.xclient.message_type == wmProtocols)
                {
                    static Atom wmDeleteWindow = X11::Atom("WM_DELETE_WINDOW");
                    PM_UNUSED
                    static Atom wmPing = X11::Atom("_NET_WM_PING", true);
            ...
            case ConfigureNotify:
            {
                ...
                {
                    ::Window dummy;
                    XTranslateCoordinates(X11::Display(), window->m_Parent,
            ...
            case EnterNotify:
            {
                auto window = WindowMap()[event.xcrossing.window];
                if (!window) return;
            ...
            case FocusIn:
            {
                auto window = WindowMap()[event.xfocus.window];
                if (!window) return;
            ...
            case MotionNotify:
            {
                auto window = WindowMap()[event.xmotion.window];
                if (!window) return;
