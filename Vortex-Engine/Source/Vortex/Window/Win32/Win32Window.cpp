...
namespace Vortex
{
        ...
        : Window(specification)
    {
        ...
        auto         styleEx = WindowStyleEx();

        Ref<Monitor> monitor = specification.Monitor;
        PM_UNUSED
        GLFWmonitor* monitorHandle
            = monitor ? std::any_cast<GLFWmonitor*>(monitor->NativeHandle())
                      : nullptr;
        ...
        {
            PM_UNUSED
            StringView monitorName = monitor->Name();
            VideoMode  currentMode = specification.Monitor->CurrentVideoMode();
            PM_UNUSED
            u32 bitsPerPixel = currentMode.RedBits + currentMode.GreenBits
                             + currentMode.BlueBits;
            PM_UNUSED
            u32 redBits = currentMode.RedBits;
            PM_UNUSED
            u32 greenBits = currentMode.GreenBits;
            PM_UNUSED
            i32 blueBits = currentMode.BlueBits;
            PM_UNUSED
    ...
    String      Win32Window::Title() const noexcept { return m_Data.Title; }
    Math::Vec2i Win32Window::Position() const noexcept
    {
        POINT pos = {0, 0};
    ...
    Math::Vec2i Win32Window::Size() const noexcept
    {
        RECT area;
    ...
    Math::Vec2i Win32Window::FramebufferSize() const noexcept { return Size(); }
    Math::Vec2f Win32Window::ContentScale() const noexcept
    {
        Math::Vec2f    ret;
    ...
    f32 Win32Window::Opacity() const noexcept
    {
        BYTE  alpha = 0;
        DWORD flags = 0;
    ...
    void Win32Window::SetSize(const Math::Vec2i& size) noexcept
    {
    ...
    void Win32Window::SetCursorPosition(Math::Vec2d position) noexcept
    {
        POINT pos
    ...
    DWORD Win32Window::WindowStyleEx()
    {
...
#pragma region callbacks
            ...
            [](GLFWwindow* handle, i32 key, i32 scancode, i32 action, i32 mods)
        {
            ...
            switch (action)
            {
                case GLFW_PRESS:
                    ...
                    window->m_Data.Keys[ToUnderlying(keycode)] = true;
                    break;
                case GLFW_RELEASE:
                    KeyReleasedEvent(window, keycode);
                    window->m_Data.Keys[ToUnderlying(keycode)] = false;
                    break;
            ...
            = [](GLFWwindow* handle, i32 button, i32 action, i32 mods)
        {
            ...
            if (action == GLFW_PRESS)
            {
                ...
                window->m_Data.MouseButtons[ToUnderlying(mousecode)] = true;
            }
            else if (action == GLFW_RELEASE)
            {
                MouseButtonReleasedEvent(window, mousecode);
                window->m_Data.MouseButtons[ToUnderlying(mousecode)] = false;
            }
        };
        PM_UNUSED
...
#undef VtGetWindow
        ...
        switch (msg)
        {
            ...
            case WM_KEYDOWN:
            {
                ...
                if (((u32)lParam & (1 << 31)) == 0)
                {
                    ...
                    m_Data.Keys[ToUnderlying(key)] = true;
                }
                else {
    ...
    {
        Win32Window* window = WindowMap()[hWnd];
                                    ...
                                    i32 height)
    {
        ...
        {
            const i32 currentDiff = Prism::Math::Absolute(
                                          ...
                                          i32 yhot, bool icon)
    {
        ...
        HBITMAP mask
            = CreateBitmap(image.Width(), image.Height(), 1, 1, nullptr);
        if (!mask)
        {
