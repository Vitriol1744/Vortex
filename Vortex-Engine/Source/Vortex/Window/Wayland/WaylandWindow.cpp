...
namespace Vortex
{
    ...
    {
        const i32 stride = icon->Width() * 4;
        const i32 length = icon->Width() * icon->Height() * 4;
        ...
        wl_shm_pool* pool = wl_shm_create_pool(Wayland::Shm(), fd, length);
        close(fd);

        unsigned char* source = icon->Pixels();
        unsigned char* target = reinterpret_cast<unsigned char*>(data);

        for (int i = 0; i < icon->Width() * icon->Height(); i++, source += 4)
        {
        ...
        struct wl_buffer* buffer
            = wl_shm_pool_create_buffer(pool, 0, icon->Width(), icon->Height(),
    ...
    {
        PM_UNUSED
        const char*    s_ProxyTag           = "Proxy tag";
        ...
        : Window(specs)
    {
        ...
        PM_UNUSED
        i32 width = specs.VideoMode.Width;
        PM_UNUSED
        i32 height = specs.VideoMode.Height;
        PM_UNUSED
        const char* title  = specs.Title.Raw();
        ...
        auto surface = wl_compositor_create_surface(Wayland::Compositor());
        VtCoreSlowAssert(surface);

        wl_surface_set_user_data(surface, this);
        m_WmSurface = xdg_wm_base_get_xdg_surface(Wayland::WmBase(), surface);
        ...
        if (Wayland::AlphaModifier())
            m_AlphaModifierSurface = wp_alpha_modifier_v1_get_surface(
                Wayland::AlphaModifier(), m_WindowHandle);
    ...
    String      WaylandWindow::Title() const noexcept { return m_Data.Title; }
    Math::Vec2i WaylandWindow::Position() const noexcept
    {
        // NOTE(v1tr10l7): wayland doesn't provide a way to retrieve window
        // position
        return {0, 0};
    }
    inline Math::Vec2i WaylandWindow::Size() const noexcept
    {
        return Math::Vec2i(m_Data.VideoMode.Width, m_Data.VideoMode.Height);
    }
    Math::Vec2i WaylandWindow::FramebufferSize() const noexcept
    {
        return Size();
    }
    Math::Vec2f WaylandWindow::ContentScale() const noexcept
    {
        return {0.0f, 0.0f};
    }
    f32  WaylandWindow::Opacity() const noexcept { return 1.0f; }
    ...
    void WaylandWindow::SetTitle(StringView title)
    {
        m_Data.Title = title;
        xdg_toplevel_set_title(m_TopLevel, title.Raw());
    }
    void WaylandWindow::SetIcon(const Icon* icons, usize count)
    {
        auto iconManager = Wayland::IconManager();
        if (iconManager)
        {
    ...
    void WaylandWindow::SetSize(const Math::Vec2i& size) noexcept
    {
        (void)size;
        VtTodo();
    }
    ...
    void WaylandWindow::SetCursorPosition(Math::Vec2d position) noexcept
    {
        VtUnused(position);
        VtCoreWarn(
                                           ...
                                           wl_fixed_t yOffset)
    {
        ...
        WaylandWindow* window = WaylandWindow::WindowMap()[surface];
        s_Serial              = serial;
                                            ...
                                            wl_fixed_t yOffset)
    {
        ...
        if (window) window->m_Data.MousePosition = Math::Vec2d(xpos, ypos);
                                          ...
                                          u32 state)
    {
        ...
        if (state == WL_KEYBOARD_KEY_STATE_PRESSED)
        {
            ...
            if (xkb_state_key_get_syms(Wayland::XkbData().State, keycode,
                                       &keysyms))
            {
