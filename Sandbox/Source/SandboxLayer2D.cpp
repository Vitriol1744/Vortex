...
void SandboxLayer2D::OnAttach()
{
    ...
    // specification.Window = Application::Get()->Window();
    ...
    VtInfo("Frames in flight: {}", Renderer::Configuration().MaxFramesInFlight);
    ...
    Math::Vec2  mousePos        = window.CursorPosition();

    static auto onMouseScrolled = [](Window*, f64, f64 deltaY) -> bool
    {
        s_ZoomLevel += deltaY * Application::Get()->DeltaTime();
        return false;
    };
...
void SandboxLayer2D::OnUpdate(Timestep deltaTime)
{
    VtProfileFunction();
    auto& window = Application::Get()->Window();
    f32   aspectRatio
    ...
    if (Keyboard::IsKeyPressed(KeyCode::eA))
        s_Translation.x += s_MovementSpeed * deltaTime;
    else if (Keyboard::IsKeyPressed(KeyCode::eD))
        s_Translation.x -= s_MovementSpeed * deltaTime;
    else if (Keyboard::IsKeyPressed(KeyCode::eW))
        s_Translation.y -= s_MovementSpeed * deltaTime;
...
void SandboxLayer2D::OnRender()
{
    ...
    Math::Vec4 redColor(1.0f, 0.0f, 0.0f, 1.0f);
    Math::Vec4 blueColor(0.0f, 0.0f, 1.0f, 1.0f);

    Math::Vec3 scale(0.05f, 0.05f, 0.5f);

    for (f32 y = -5.0f; y < 5.0f; y += 0.5f)
    {
        for (f32 x = -5.0f; x < 5.0f; x += 0.5f)
        {
            Math::Vec2 pos(x * 0.11f, y * 0.11f);
            // Mat4 transform = glm::translate(Mat4(1.0f), pos) * scale;
            Math::Vec4 color = Math::Vec4((x + 5.0f) / 10.0f, 0.4f,
                                          (y + 5.0f) / 10.0f, 0.2f);
...
void SandboxLayer2D::OnImGuiRender()
{
    ...
    ImGui::Text("FPS: %lu", Application::Get()->FPSCounter());
    ImGui::Text("Delta Time: %f", Application::Get()->DeltaTime());
    ...
    ImGui::Text("VRAM: %luMB/%luMB", Renderer::MemoryUsage() / 1024 / 1024,
                Renderer::MemoryBudget() / 1024 / 1024);
    ImGui::Text("Memory Budget: %luMB", Renderer::MemoryBudget() / 1024 / 1024);
    ...
    static auto         startTime = Time::CurrentTime();
    f32                 time      = Time::CurrentTime() - startTime;

    UniformBufferObject ubo{};
    ubo.Model          = glm::rotate(Mat4(1.0f), time * glm::radians(90.0f),
                                     Math::Vec3(0.0f, 0.0f, 1.0f));
    /*Mat4 view = glm::lookAt(Math::Vec3(2.0f, 2.0f, 2.0f), Math::Vec3(0.0f,
    0.0f, 0.0f), Math::Vec3(0.0f, 0.0f, 1.0f));

    Mat4 projection      = glm::perspective(
        glm::radians(45.0f),
        extent.width / (f32)extent.height, 0.1f, 10.0f);*/

    ubo.ViewProjection = s_Camera.ViewProjection();
